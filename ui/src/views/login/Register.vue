<template>
  <main class="login-page">
    ...
    <section class="login-card reveal-block delay-1">
      ...
      <h2>{{ t('app.name') }}</h2>
      <div class="form-stack" @keyup.enter="handleRegister">
        <label>
        ...
        <label>
          <span>昵称</span>
          <el-input v-model="name" placeholder="选填" size="large" />
        </label>
        <label>
        ...
        <label>
          <span>确认密码</span>
          <el-input v-model="confirmPassword" placeholder="请再次输入密码" type="password" show-password size="large" />
        </label>
        <button class="primary-action" :disabled="loading" @click="handleRegister">
          {{ loading ? '注册中...' : '注册' }}
        </button>
        <p class="switch-link">
...
const handleRegister = async () => {
  ...
  loading.value = true
  try {
    // 直接使用 fetch 绕过 axios 拦截器和路由守卫
    const resp = await fetch(import.meta.env.VITE_APP_URL + '/user/register', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        account: account.value,
        password: password.value,
        name: name.value || account.value
      })
    })
